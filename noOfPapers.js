// We accept two inputs, one for the number of students and one for the number of pages per student required and calculate the total pages used
// We set two parameters, one for number of students and second one for pages per student. If either parameter is set to value of zero or is less than zero, we return 0
// We return the value of number of total copies required and if no copies required, we return 0
//  Our testcases include; n = 6, m = 4 : 24, n = 7, m = 0 : 0, n = -5, m = 6 : 0
// We initialize our function and declare our two parameters; n(no. of students) and m(no. of pages per student), check if both values are greater than 0 and return product of n and m;


function paperwork(n, m) {
    return n > 0 && m > 0 ? n * m : 0
  }
 
  paperwork(6, 4) // -> 24
  paperwork(7, 0) // -> 0
  paperwork(-5, 6) // -> 0
